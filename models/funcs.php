...
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT aceptado
		FROM ".$db_table_prefix."expedientestaff
		WHERE
		idStaffExpediente = ?
		ORDER BY 
		idStaffExpediente ASC
		LIMIT 1");
	$stmt->bind_param("i", $id);	
	$stmt->execute();
	$stmt->store_result();
...
function deleteStaffExpediente($id){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("DELETE FROM ".$db_table_prefix."expedientestaff
		WHERE idStaffExpediente = ?");
		$stmt->bind_param("i", $id);
		$stmt->execute();
		$stmt->close();
		return true;
...
function fetchAllStaffs(){
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		email,
		celular
		FROM ".$db_table_prefix."staff");
	$stmt->execute();
	$stmt->bind_result($id, $apellidoP, $apellidoM, $nombre, $sexo, $fechaNac, $universidad, $carrera, $fechaIngreso, $estatus, $campParticipados, $rango, $clave, $email, $celular);
	while ($stmt->fetch()){
		$row[] = array('id' => $id,'apellidoP' => $apellidoP,'apellidoM' => $apellidoM,'nombre' => $nombre,'sexo' => $sexo,'fechaNac' => $fechaNac,'universidad' => $universidad,'carrera' => $carrera,'fechaIngreso' => $fechaIngreso,'estatus' => $estatus,'campParticipados' => $campParticipados,'rango' => $rango,'clave' => $clave,'email' => $email,'celular' => $celular);
	}
	$stmt->close();
	return ($row);
}
...
function updateStaff($apellidoP,$apellidoM,$nombre,$sexo,$fechaNac,$universidad,$carrera,$fechaIngreso,$estatus,$campParticipados,$rango,$clave,$email,$celular,$id){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."staff
		SET apellidoP = ?,
		 apellidoM = ?,
		 nombre = ?,
		 sexo = ?,
		...
		LIMIT 1
		");
	$stmt->bind_param("sssisssssissssi", $apellidoP,$apellidoM,$nombre,$sexo,$fechaNac,$universidad,$carrera,$fechaIngreso,$estatus,$campParticipados,$rango,$clave,$email,$celular,$id);
	$result = $stmt->execute();
	$stmt->close();
	return $result;

}
function createStaffExpediente($primeraEn,$evaluador,$aceptado,$conclusiones,$cartaPadres,$cartaCompromiso,$campSeleccion,$pruebaFisica,$cartaRecomendacion,$reporteNeg,$habilidadesUtiles,$staff_id){
	global $mysqli,$db_table_prefix;
...
function createStaff($apellidoP,$apellidoM,$nombre,$sexo,$fechaNac,$universidad,$carrera,$fechaIngreso,$estatus,$campParticipados,$rango,$clave,$email,$celular){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("INSERT INTO ".$db_table_prefix."staff
		(apellidoP,
		 apellidoM,
		 nombre,
		 sexo,
	...
	VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
		");
	$stmt->bind_param("sssisssssiisss", $apellidoP,$apellidoM,$nombre,$sexo,$fechaNac,$universidad,$carrera,$fechaIngreso,$estatus,$campParticipados,$rango,$clave,$email,$celular);
	$result = $stmt->execute();
	$stmt->close();
	return $result;
...
function deleteStaff($id){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("DELETE FROM ".$db_table_prefix."staff 
		WHERE idStaff = ?");
		$stmt->bind_param("i", $id);
		$stmt->execute();
		$stmt->close();
...
function fetchAllMontanistas(){
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		noFolio,
		colegio
		FROM ".$db_table_prefix."montanistas");
	$stmt->execute();
	$stmt->bind_result($id, $apellidoP, $apellidoM, $nombre, $nickname, $fechaNac, $sexo, $pais, $estado, $ciudad, $calle, $noExt, $noInt, $colonia, $cp, $telefono, $celular, $email, $nombreMama, $emailM, $nombrePapa, $emailP, $diasAltikamp, $nivelExpertiz, $obMed, $seguroGastosMed, $observaciones, $clave, $contactoEmergencia, $telefonoEmergencia, $noFolio, $colegio);
		

	
	while ($stmt->fetch()){
		$row[] = array('id' => $id,'apellidoP' => $apellidoP,'apellidoM' => $apellidoM,'nombre' => $nombre,'nickname' => $nickname,'fechaNac' => $fechaNac,'sexo' => $sexo,'pais' => $pais,'estado' => $estado,'ciudad' => $ciudad,'calle' => $calle,'noExt' => $noExt,'noInt' => $noInt,'colonia' => $colonia,'cp' => $cp,'telefono' => $telefono,'celular' => $celular,'email' => $email,'nombreMama' => $nombreMama,'emailM' => $emailM,'nombrePapa' => $nombrePapa,'emailP' => $emailP,'diasAltikamp' => $diasAltikamp,'nivelExpertiz' => $nivelExpertiz,'obMed' => $obMed,'seguroGastosMed' => $seguroGastosMed,'observaciones' => $observaciones,'clave' => $clave,'contactoEmergencia' => $contactoEmergencia,'telefonoEmergencia' => $telefonoEmergencia,'noFolio' => $noFolio,'colegio' => $colegio);
	}
	$stmt->close();
	return ($row);
}
function fetchMontanistaDetails($id){
	if($id!=NULL) {
		$column = "idMontanista";
		$data = $id;
	}
...
function updateMontanista($apellidoP,$apellidoM,$nombre,$nickname,$fechaNac,$sexo,$pais,$estado,$ciudad,$calle,$noExt,$noInt,$colonia,$cp,$telefono,$celular,$email,$nombreMama,$emailM,$nombrePapa,$emailP,$diasAltikamp,$nivelExpertiz,$obMed,$seguroGastosMed,$observaciones,$clave,$contactoEmergencia,$telefonoEmergencia,$noFolio,$colegio,$id){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."montanistas
		SET apellidoP = ?,
		 apellidoM = ?,
		 nombre = ?,
		 nickname = ?,
		...
		LIMIT 1
		");
	$stmt->bind_param("sssssissssiisisssssssisssssssssi", $apellidoP,$apellidoM,$nombre,$nickname,$fechaNac,$sexo,$pais,$estado,$ciudad,$calle,$noExt,$noInt,$colonia,$cp,$telefono,$celular,$email,$nombreMama,$emailM,$nombrePapa,$emailP,$diasAltikamp,$nivelExpertiz,$obMed,$seguroGastosMed,$observaciones,$clave,$contactoEmergencia,$telefonoEmergencia,$noFolio,$colegio,$id);
	$result = $stmt->execute();
	$stmt->close();
	return $result;

}
function createMontanista($apellidoP,$apellidoM,$nombre,$nickname,$fechaNac,$sexo,$pais,$estado,$ciudad,$calle,$noExt,$noInt,$colonia,$cp,$telefono,$celular,$email,$nombreMama,$emailM,$nombrePapa,$emailP,$diasAltikamp,$nivelExpertiz,$obMed,$seguroGastosMed,$observaciones,$clave,$contactoEmergencia,$telefonoEmergencia,$noFolio,$colegio){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("INSERT INTO ".$db_table_prefix."montanistas
		(idMontanista,
		 apellidoP,
		 apellidoM,
		 nombre,
	...
	VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
		");
	$stmt->bind_param("sssssissssiisisssssssisssssssss", $apellidoP,$apellidoM,$nombre,$nickname,$fechaNac,$sexo,$pais,$estado,$ciudad,$calle,$noExt,$noInt,$colonia,$cp,$telefono,$celular,$email,$nombreMama,$emailM,$nombrePapa,$emailP,$diasAltikamp,$nivelExpertiz,$obMed,$seguroGastosMed,$observaciones,$clave,$contactoEmergencia,$telefonoEmergencia,$noFolio,$colegio);
	$result = $stmt->execute();
	$stmt->close();
	return $result;

}
function deleteMontanista($id){
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("DELETE FROM ".$db_table_prefix."montanistas 
		WHERE idMontanista = ?");
		$stmt->bind_param("i", $id);
		$stmt->execute();
		$stmt->close();
...
function fetchEmployeesExpedient($id){
...
function getTemplateFiles()
{
	...
	$languages = glob($directory . "altikamp/");
	//print each file name
	return $languages;
}
